...
cris.connect = function()
{
    ...
    odbcDriverConnect(paste('driver={SQL Server};server=BRHNSQL009;UID=', USER, ';PWD=', PASSWORD, sep=""))
...
)
{
    ...
    for (i in 1:length(filenames)) {
        params[paste("file_", i, sep="")] = list( fileUpload(filename = filenames[i]) )
...
cris.cpft.get_patients_by_startdate_diagnosis = function(dbhandle, startdate, condition)
{
    ...
    x = sqlQuery(dbhandle, paste("
        SELECT
            ...
            END AS dod
            , CASE
                WHEN M.truncated_dttm = 0 THEN CAST('", CRIS_EXPORT_DATE_AS_TEXT, "' AS DATE) /* not dead */ /* not CURRENT_TIMESTAMP as CRIS export not nightly */
                ELSE CAST(M.truncated_dttm AS DATE) /* dead */
            END AS final_date
            , (
...
cris.cpft.admissions_between_inclusive = function(admissions_df, patient_id, startdate, enddate)
{
    ...
    # Do not use "id" as the function argument -- treat as a reserved word (esp. in the next statement)
    # Passing patient_id = NA means calculate the average for all patients represented
    if (is.na(patient_id)) {
        ...
        ifelse(
            ...
            ifelse(
                ...
                ifelse(
                    ...
                    as.integer(enddate - startdate), # admission,START,___,END,discharge
                    as.integer(discharge_date - startdate) # admission,START,___,discharge,END
                ),
                ifelse(
                    enddate < discharge_date,
                    as.integer(enddate - admission_date), # START,admission,___,END,discharge
                    as.integer(discharge_date - admission_date) # START,admission,___,discharge,END
                )
            )
        )
    )
...
cris.cpft.admissions_as_binary = function(admissions_df, patient_id, alldates)
{
    ...
    for (i in seq_along(admission_dates)) {
        ...
        start_index = ifelse(adm < first_date, 1, which(alldates == admission_dates[i]) )
        end_index   = ifelse(dis > last_date, n, which(alldates == discharge_dates[i]) )
    ...
    , EXCLUDE_YEARS_NOT_WITHIN_EACH_PATIENT_DATE_RANGE = TRUE)
{
    ...
    if (EXCLUDE_YEARS_NOT_WITHIN_EACH_PATIENT_DATE_RANGE) {
        d = subset(d, first_diagnosis_date <= year_starts & year_ends <= final_date)
    }
    n = nrow(d)
    id = d$id
...
cris.cpft.attempt_daily_dose = function(druginfo_df)
{
    within(druginfo_df, {
        ...
        conversion_to_mg = ifelse(
            dose_unit == "mg" | dose_unit == "mgs" | dose_unit == "mgm" | dose_unit == "Mg" | dose_unit == "MG" | dose_unit == "mgr",
            1,
            ifelse(
                dose_unit == "mcg" | dose_unit == "mcgs" | dose_unit == "micrograms" | dose_unit == "microgrammes" | dose_unit == "microgram" | dose_unit == "microg",
                1/1000,
                ifelse(
                    dose_unit == "g" | dose_unit == "gm" | dose_unit == "gram" | dose_unit == "grams" | dose_unit == "Gm" | dose_unit == "G" | dose_unit == "gr",
                    1000,
                    NA
                )
            )
...
{
    # Assumes you used pvals.fnc
    mcmcdata = pvals_fnc_result$fixed
    if (strip_true) {
        rownames(mcmcdata) = gsub("TRUE", "", rownames(mcmcdata))
    }
    d = data.frame(
        names = as.factor(rownames(mcmcdata))
        # , means = as.numeric(mcmcdata$Estimate) # no, that's the non-MCMC estimate
        , means = as.numeric(mcmcdata$MCMCmean)
        , lower = as.numeric(mcmcdata$HPD95lower)
        , upper = as.numeric(mcmcdata$HPD95upper)
    )
    d = within(d, {
        if (sort_by_effect_size) {
            names = reorder(names, means, FUN=mean) # without FUN=mean it sometimes does nothing. Try: a = as.factor(c("a","b","c")); reorder(a, c(3,2,1)); reorder(a, c(3,2,1), FUN=mean)
        }
        # Reverse order (so Y axis reads top-down, not bottom-up)
        names = factor(names, levels = rev(levels(names)) )
        # Flag "significant" effects
        hpd_excludes_zero = sign(lower) == sign(upper)
    })
    ...
    if (visual_style == 1) {
        return(
            ggplot(
                data = d
                , aes(y = names, x = means, colour = factor(hpd_excludes_zero))
            )
        ...
        return(
            ...
            + geom_vline(xintercept = 0, colour = "grey50")
            + geom_errorbarh( aes(xmin = lower, xmax = upper) )
            + geom_point(size = 5, shape = 21, mapping = aes(fill = factor(hpd_excludes_zero)))
            + xlab(xlab)
            + ylab("")
...
)
{
    ...
    if (colnames(coefs)[3] != "z value") {
        cat("need an lmer object producing Z values\n")
        return()
    }
    if (strip_true) {
    ...
    d = data.frame(
        names = as.factor(rownames(coefs))
        , means = coefs[,"Estimate"]
        , se = coefs[,"Std. Error"]
        , z = coefs[,"z value"] # ... which is mean/se (confirmed empirically), since df=1
        , p = coefs[,"Pr(>|z|)"]
    )
    d = within(d, {
        if (sort_by_effect_size) {
            names = reorder(names, means, FUN=mean)
        }
        # Reverse order (so Y axis reads top-down, not bottom-up)
        names = factor(names, levels = rev(levels(names)) )

        # Calculate confidence intervals (two-tailed)
        lower = qnorm(alpha/2, means, se)
        upper = qnorm(1 - alpha/2, means, se)

        # Flag "significant" effects
        ci_excludes_zero = sign(lower) == sign(upper)
    })
    ...
    if (visual_style == 1) {
        return(
            ggplot(
                data = d
                , aes(y = names, x = means, colour = factor(ci_excludes_zero))
            )
            + geom_vline(xintercept = 0, colour = "grey50")
            + geom_errorbarh( aes(xmin = lower, xmax = upper) )
            + geom_point(size = 5)
        ...
        return(
            ...
            + geom_vline(xintercept = 0, colour = "grey50")
            + geom_errorbarh( aes(xmin = lower, xmax = upper) )
            + geom_point(size = 5, shape = 21, mapping = aes(fill = factor(ci_excludes_zero)))
            + coord_cartesian(xlim = limits)
            + xlab(xlab)
            + ylab("")
            + scale_fill_manual(values = c("white", "black"), guide = FALSE)
...
cris.cpft.print_simple_patient_summary = function(patient_df)
{
    ...
    mean_age_at_death = mean(patient_df$age_at_death[!is.na(patient_df$age_at_death)])
    mean_age_at_death_male = mean(patient_df$age_at_death[!is.na(patient_df$age_at_death) & patient_df$sex == "Male"])
    mean_age_at_death_female = mean(patient_df$age_at_death[!is.na(patient_df$age_at_death) & patient_df$sex == "Female"])
    chisq_sex = chisq.test( c(n_male, n_female) )
    patient_df_sex_mf = subset(patient_df, sex=="Male" | sex=="Female") # could also be not known/not specified/X...
...
cris.cpft.admissions_by_year_plot_by_patient = function(admissions_by_patient_year_df, firstyear, lastyear)
{
    ...
    return(
        ...
        + geom_line()
        + scale_y_continuous("Mean admission days per patient per year", limits = c(0,max(SUMMARY_ADMDAYS_BY_YEAR$mean_admission_days_per_pt)) )
...
cris.cpft.admissions_by_year_plot_as_institution_proportion = function(admissions_by_patient_year_df, available_beds_by_year_df, firstyear, lastyear)
{
    ...
    return(
        ...
        + geom_line()
        + scale_y_continuous("Proportion of available bed-days used", limits = c(0,1) )
        + scale_x_continuous("Year", limits = c(firstyear, lastyear) )
        + theme_bw()
    )
}
...
cris.cpft.simple_drug_stats = function(drugname, druginfo, patient_df)
{
    ...
    pval_use_by_sex = ifelse(n_used > 0, chisq.test(P$used, P$sex)$p.value, NA)
    pval_use_by_death = ifelse(n_used > 0, chisq.test(P$used, P$died)$p.value, NA)
...
cris.cpft.drug_use_by_diagnosis = function(drugname, druginfo_df_diagnosis, patient_df_diagnosis, druginfo_df_no_diagnosis, patient_df_no_diagnosis)
{
    ...
    pval_use_by_diagnosis = ifelse(
        n_used_diagnosis > 0 && n_used_no_diagnosis > 0 && n_diagnosis > 0 & n_no_diagnosis
...
cris.drug_mirror_plot = function(MIRROR_SUMMARY, DRUGLIST = NA, corrected = FALSE, sort_by_effect_size = TRUE, title = "", visual_style = 1)
{
    ...
    MIRROR_SUMMARY = subset(MIRROR_SUMMARY, !is.na(pvalue_adm_days) & !is.na(mean_adm_days_per_year_before) & !is.na(mean_adm_days_per_year_after))
    MIRROR_SUMMARY = within(MIRROR_SUMMARY, {
        # Append n
        drugname = paste(drugname, " (n=", n_pts, ", ", round(mean_age, digits=1), "y, ", n_male, " M)", sep="")
        # Which sort of analysis?
        if (corrected) {
            depvar = mean_corrected_adm_days_per_year_change
            hci = hci95_corrected_adm_days_change
            if (sort_by_effect_size) {
                drugname = reorder(drugname, mean_corrected_adm_days_per_year_change, FUN=mean) # Sort drug factor order
            }
        }
        else {
            ...
            if (sort_by_effect_size) {
                drugname = reorder(drugname, mean_adm_days_per_year_change, FUN=mean) # Sort drug factor order
            }
        }
        # Reverse order (so Y axis reads top-down, not bottom-up)
...
cris.age_at_death_by_year_diagnosis = function(dead_patient_df, ci = 0.95)
{
    ...
    return(
        ...
        + geom_ribbon(aes(ymin=mean_age_at_death - hci, ymax = mean_age_at_death + hci, fill=sex), alpha=0.1)
        + geom_line(size=2)
...
cris.cpft.get_honos = function(dbhandle, idlist)
{
    ...
    r = within(r, {
        ...
        total = ifelse(core_complete, q1 + q2 + q3 + q4 + q5 + q6 + q7 + q8 + q9 + q10 + q11 + q12, NA)
    })

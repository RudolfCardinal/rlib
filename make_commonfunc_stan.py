...
AUROC_NOTES = """
    ...
    */
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // METHOD 1: TOO SLOW (in example: 2000 s/1000 iterations overall).
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    real aurocAV(int[] binary_outcome, vector probability)
    {
        ...
        int outcome_i;
        int outcome_j;
        real p_for_outcome_zero;
        real p_for_outcome_one;
        real score = 0;
        int n_unequal_pairs = 0;

        // Checks
        if (n != num_elements(probability)) {
            reject("Incompatible arguments");
        }
        ...
        for (i in 1:n - 1) {
            for (j in i + 1:n) {
                // That enumerates all combinations once.
                outcome_i = binary_outcome[i];
                outcome_j = binary_outcome[j];
                if (outcome_i == outcome_j) {
                    continue;  // We only want mismatched pairs.
                }
                n_unequal_pairs += 1;
                if (outcome_i == 0) {
                    p_for_outcome_zero = probability[i];
                    p_for_outcome_one = probability[j];
                } else {
                    p_for_outcome_zero = probability[j];
                    p_for_outcome_one = probability[i];
                }
                // Now score:
                if (p_for_outcome_one > p_for_outcome_zero) {
                    score += 1.0;
                } else if (p_for_outcome_one > p_for_outcome_zero) {
                    score += 0.5;
                }
                // otherwise, it's a loss.
            }
        }
        if (n_unequal_pairs == 0) {
            reject("'binary_outcome' must have two levels to calculate AUROC")
        }
        // The AUROC is:
        print("AUROC: ", score /  n_unequal_pairs);
        return score /  n_unequal_pairs;
        // The combination of float and integer arithmetic promotes
        // everything to float:
        // https://mc-stan.org/docs/2_18/functions-reference/int-arithmetic.html
    }
    */

    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // METHOD 2: TOO SLOW, e.g. 20,000-30,000 s/1000 iterations overall
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

    /*

    int nUnequalPairs(int[] binary_outcome)
    {
        // For a binary outcome array, calculates the number of pairs of
        // outcomes that are discrepant (one being 0, the other being 1).
        //
        // Part of the precalculation for AUROC. Used by
        // outcomeUnequalPairZeroIndexes().

        int n = num_elements(binary_outcome);
        int n_ones = sum(binary_outcome);
        int n_zeros = n - n_ones;
        int n_unequal_pairs = n_ones * n_zeros;
        if (n_unequal_pairs == 0) {
            reject("'binary_outcome' must have two levels to calculate AUROC")
        }
        return n_unequal_pairs;
    }

    int[,] unequalOutcomePairIndexes(int[] binary_outcome)
    {
        // Precalculate this in "transformed data" if you want any decent speed
        // to the AUROC calculation.
        // Returns an integer array: int indexes[n_unequal_pairs, 2].
        // For a given pair, indexes[pair, 1] is the index of the "zero"
        // outcome and indexes[pair, 2] is the index of the "one" outcome.

        int n_outcomes = num_elements(binary_outcome);
        int n_unequal_pairs = nUnequalPairs(binary_outcome);
        int indexes[n_unequal_pairs, 2];
        int pair = 0;

        // Calculation
        for (i in 1:n_outcomes - 1) {
            for (j in i + 1:n_outcomes) {
                if (binary_outcome[i] == binary_outcome[j]) {
                    continue;  // We only want mismatched pairs.
                }
                pair += 1;
                if (binary_outcome[i] == 0) {
                    indexes[pair, 1] = i;
                    indexes[pair, 2] = j;
                } else {
                    indexes[pair, 1] = j;
        ...
        print("unequalOutcomePairIndexes: n_unequal_pairs = ", n_unequal_pairs);
        return indexes;
    }

    real aurocAV(int[] binary_outcome,
                 vector probability,
                 int[,] unequal_outcome_pair_indexes)
    {
        int n_unequal_pairs = dims(unequal_outcome_pair_indexes)[1];
        vector[n_unequal_pairs] scores = rep_vector(0, n_unequal_pairs);
        real p_for_outcome_zero;
        real p_for_outcome_one;

        // Checks
        if (num_elements(binary_outcome) != num_elements(probability)) {
            reject("Incompatible arguments");
        }

        // Calculation
        for (i in 1:n_unequal_pairs) {
            p_for_outcome_zero = probability[unequal_outcome_pair_indexes[i, 1]];
            p_for_outcome_one = probability[unequal_outcome_pair_indexes[i, 2]];
            // Now score:
            if (p_for_outcome_one > p_for_outcome_zero) {
                scores[i] = 1.0;
            } else if (p_for_outcome_one > p_for_outcome_zero) {
            ...
            // ... otherwise, the score was preset to zero.
        }
        return mean(scores);
    }
    ...
    */
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // METHOD 3: MUCH, MUCH BETTER (e.g. 37 s/2000 iterations in our example).
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
